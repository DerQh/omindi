  ...
  [...orders]
    ...
    .forEach((o) => {
      ...
      if (period === "day")
        ...
        key = w.toLocaleDateString("en-US", { month: "short", day: "numeric" });
...
const Admin = () => {
  ...
  const chartData = useMemo(
    () => groupByPeriod(stats?.ordersRaw, period),
    [stats?.ordersRaw, period],
  );
  ...
  const avgOrderValue = useMemo(() => {
    ...
    return ords.length
      ? Math.round((stats?.totalRevenue ?? 0) / ords.length)
      : 0;
  }, [stats]);
  ...
  return (
    <>
      ...
      {/* Mobile tab bar — sits outside Shell so it shares the same sticky context as AppNavbar */}
      <MobileTabBar>
        {NAV.map((n) => (
          <MobileTab
            key={n.id}
          ...
          >
            ...
            {badges[n.id] > 0 && (
              <MobileTabBadge>{badges[n.id]}</MobileTabBadge>
            )}
          </MobileTab>
        ))}
      </MobileTabBar>
      ...
      <Shell>
            ...
            <SideNav>
              {NAV.map((n) => (
                ...
                >
                  ...
                  {badges[n.id] > 0 && (
                    <SideNavBadge>{badges[n.id]}</SideNavBadge>
                  )}
                </SideNavItem>
              ))}
            </SideNav>
        ...
        <Main>
          <MainInner>
            ...
            {tab === "overview" && (
              <Fade key="overview">
                <PageHeading>Sales Overview</PageHeading>
                ...
                <ChartCard>
                  ...
                  {loadingStats || !chartData.length ? (
                    <ChartEmpty>
                      {loadingStats ? "Loading…" : "No data yet."}
                    </ChartEmpty>
                  ) : (
                        ...
                        >
                          ...
                          >
                            {statusDistribution.map((s, i) => (
                              <Cell
                                key={s.name}
                                fill={STATUS_META[s.name]?.dot ?? "#9ca3af"}
                              />
                            ))}
                          </Bar>
                        </BarChart>
                ...
                {topCategories.length > 0 && (
                  <ChartCard>
                    <ChartCardHeader>
                      <ChartCardTitle>Top Listing Categories</ChartCardTitle>
                    </ChartCardHeader>
                    <CatGrid>
                      {topCategories.map((c, i) => (
                        <CatRow key={c.name}>
                          ...
                          <CatBar>
                            <CatFill
                              style={{
                                width: `${(c.count / topCategories[0].count) * 100}%`,
                                background: CAT_COLORS[i % CAT_COLORS.length],
                              }}
                            />
                          </CatBar>
                      ...
                      <tbody>
                        {(orders ?? []).slice(0, 10).map((o) => (
                          <TR key={o.id}>
                            ...
                            <TD>
                              <strong>
                                Kes {(o.total_cost ?? 0).toLocaleString()}
                              </strong>
                            </TD>
                            <TD>
                              <PayBadge>{o.payment_method ?? "—"}</PayBadge>
                            </TD>
                            <TD>
                            ...
                            <TD>
                              <Muted>{formatSmartDate(o.created_at)}</Muted>
                            </TD>
                          </TR>
                        ))}
                        {!orders?.length && (
                          ...
                          {filteredUsers.map((u) => {
                            ...
                            return (
                              <TR key={u.id}>
                                <TD>
                                  <UserCell>
                                    {u.avatar_url ? (
                                      ...
                                      <UserInitials>
                                        {initials(u.full_name || u.farm_name)}
                                      </UserInitials>
                                    )}
                                    <UserInfo>
                                      <UserPrimary>
                                ...
                                <TD>
                                  <Muted>{u.farm_name || "—"}</Muted>
                                </TD>
                                <TD>
                                  <Muted>
                                    {u.location
                                      ? String(u.location).split(",")[0]
                                      : "—"}
                                  </Muted>
                                </TD>
                                <TD>
                                ...
                                <TD>
                                  <Muted>{formatSmartDate(u.created_at)}</Muted>
                                </TD>
                                <TD>
                                  <RoleGroup>
                                    {u.is_admin && (
                                      <RoleBadge $admin>Admin</RoleBadge>
                                    )}
                                    {isSeller && (
                                      <RoleBadge $seller>Seller</RoleBadge>
                                    )}
                                    {!u.is_admin && !isSeller && (
                                      <RoleBadge>Buyer</RoleBadge>
                                    )}
                                    {/* Toggle admin role.
            ...
            {tab === "listings" && (
              <Fade key="listings">
                ...
                {loadingLists ? (
                  <LoadMsg>Loading listings…</LoadMsg>
                ) : (
                  <DataCard>
                    <DataScroll>
                      <DataTable>
              ...
              <Fade key="orders">
                ...
                {loadingOrders ? (
                  ...
                  <DataCard>
                            ...
                            <TR key={o.id}>
                              ...
                              <TD>
                                <AddressTrunc title={o.delivery_address}>
                                  ...
                                  {o.delivery_address?.length > 30 ? "…" : ""}
                                </AddressTrunc>
                              </TD>
                              <TD>
                                <Muted>{o.mobile_no ?? "—"}</Muted>
                              </TD>
                              <TD>
                                <Muted>{formatSmartDate(o.created_at)}</Muted>
                              </TD>
                    ...
                    <TableFooter>
                      Showing {filteredOrders.length} of {orders?.length ?? 0}{" "}
                      orders
                    </TableFooter>
                  </DataCard>
                )}
                ...
                {disputedOrders.length === 0 ? (
                  <EmptyCard>
                    ...
                    <EmptySub>
                      ...
                      appear here. Resolve them by updating their status.
                    </EmptySub>
                  </EmptyCard>
                ) : (
                  <>
...
const SideBrand = styled.div`
  ...
  margin-bottom: 16px;
`;
const SideBrandName = styled.div`
  font-size: 0.9rem;
  font-weight: 800;
  color: white;
  letter-spacing: 0.05em;
`;
const SideBrandSub = styled.div`
  font-size: 0.68rem;
  color: #6b7280;
  font-weight: 500;
...
const MobileTabBar = styled.div`
  ...
  @media (max-width: 768px) {
    ...
    position: sticky;
    top: 60px; /* sits directly below the 60px AppNavbar */
    z-index: 40;
  }
`;
const MobileTab = styled.button`
  ...
  border-bottom: 2px solid
    ${({ $active }) => ($active ? "#4e9643" : "transparent")};
  cursor: pointer;
  position: relative;
`;
...
const RoleBadge = styled.span`
  ...
  color: ${({ $admin, $seller }) =>
    $admin ? "#991b1b" : $seller ? "#1e40af" : "#6b7280"};
`;
const ToggleBtn = styled.button`
  ...
  border: 1.5px solid
    ${({ $on, $blocked }) =>
      $blocked ? "#e5e7eb" : $on ? "#2f5a2a" : "#e5e7eb"};
  background: ${({ $on, $blocked }) =>
    $blocked ? "#f9fafb" : $on ? "#f0fdf4" : "white"};
  color: ${({ $on, $blocked }) =>
    $blocked ? "#d1d5db" : $on ? "#2f5a2a" : "#9ca3af"};
...
const AddressTrunc = styled.span`
  color: #6b7280;
  font-size: 0.8rem;
`;
