...
const Cart = () => {
  ...
  const subtotal = cartItems.reduce(
    (s, i) => s + (i.listings?.price ?? 0) * (i.quantity ?? 1), 0,
  );
  const TAX_RATE  = 0.16;
  ...
  const handleQty = (type, item) =>
    updateItem({ user_id: user?.id, listing_id: item.listing_id, cart_id: item.id, type });

  const handleRemove = (item_id) => {
    setRemovingId(item_id);
    deleteItem({ item_id, user_id: user?.id }, { onSettled: () => setRemovingId(null) });
  };
            ...
            <ContentGrid>
                  ...
                  {cartItems.map((item) => (
                    <ItemCard key={item.id} $removing={removingId === item.id}>
                      ...
                      <ItemContent>
                        ...
                        <ItemUnitPrice>
                          Price per {item.listings?.unit ?? "unit"}: <strong>Kes {item.listings?.price?.toLocaleString()}</strong>
                        </ItemUnitPrice>
                        <ItemBottomRow>
                          <QtyControl>
                            <QtyBtn onClick={() => handleQty("decrement", item)} disabled={isUpdating || item.quantity <= 1}>−</QtyBtn>
                            <QtyLabel>{item.quantity}{item.listings?.unit ? ` ${item.listings.unit}` : ""}</QtyLabel>
                            <QtyBtn onClick={() => handleQty("increment", item)} disabled={isUpdating}>+</QtyBtn>
                          </QtyControl>
                          <ItemTotal>
              ...
              <SummaryCol>
                <SummaryCard>
                  ...
                  <SummaryLines>
                    ...
                    {promoApplied && (
                      <SummaryLine>
                        ...
                        <SummaryVal style={{ color: "#16a34a" }}>− Kes 0</SummaryVal>
                      </SummaryLine>
                    )}
                  </SummaryLines>
                  ...
                  <DesktopCheckoutBtn
                    onClick={() => navigate("/checkout", { state: { cartItems, totalCost: subtotal } })}
                  >
                    Checkout
                  </DesktopCheckoutBtn>
                </SummaryCard>
...
const HeaderTitle = styled.h1`
  flex: 1; margin: 0;
  font-size: 1.05rem; font-weight: 700; color: #1a3318;
...
const LocationLabel = styled.div`font-size: 0.72rem; color: #9ca3af; margin-bottom: 1px;`;
const LocationValue = styled.div`font-size: 0.9rem; font-weight: 700; color: #1a3318;`;
const ChangeLocation = styled.button`
  background: none; border: none; font-size: 0.8rem;
  color: #2f5a2a; font-weight: 600; cursor: pointer;
...
const ItemImg = styled.img`
  ...
  object-fit: cover; background: #f0f7ee; flex-shrink: 0;
`;
...
const ItemName = styled.p`
  margin: 0; font-size: 0.92rem; font-weight: 700; color: #1a3318;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1;
`;

const DeleteBtn = styled.button`
  background: none; border: none; cursor: pointer; flex-shrink: 0;
  color: #ef4444; padding: 2px; transition: transform 0.12s;
  &:hover { transform: scale(1.15); }
`;

const ItemUnitPrice = styled.p`
  margin: 3px 0 0; font-size: 0.78rem; color: #9ca3af;
  strong { color: #1a3318; }
`;
...
const QtyBtn = styled.button`
  ...
  border: 1.5px solid #e0ece0; background: white;
  color: #2f5a2a; font-size: 1rem; font-weight: 700;
  cursor: pointer; display: flex; align-items: center; justify-content: center;
  transition: all 0.12s;
  &:hover:not(:disabled) { background: #2f5a2a; color: white; border-color: #2f5a2a; }
  &:disabled { opacity: 0.35; cursor: not-allowed; }
`;

const QtyLabel = styled.span`
  font-size: 0.85rem; font-weight: 700; color: #1a3318; min-width: 36px; text-align: center;
`;

const ItemTotal = styled.span`
  font-size: 0.92rem; font-weight: 800; color: #1a3318;
`;
...
const PromoInput = styled.input`
  ...
  font-size: 0.88rem; color: #1a3318; outline: none;
...
const SummaryLabel = styled.span`font-size: 0.85rem; color: #6b7280;`;
const SummaryVal   = styled.span`font-size: 0.85rem; font-weight: 600; color: #1a3318;`;
...
const TotalLabel = styled.span`font-size: 0.92rem; font-weight: 700; color: #1a3318;`;
const TotalVal   = styled.span`font-size: 0.92rem; font-weight: 800; color: #1a3318;`;
...
const DesktopCheckoutBtn = styled.button`
  ...
  @media (min-width: 768px) {
    ...
    &:hover { box-shadow: 0 6px 18px rgba(47,90,42,0.4); transform: translateY(-1px); }
...
const CheckoutBtn = styled.button`
  ...
  transition: box-shadow 0.15s, transform 0.1s;
  &:hover { box-shadow: 0 6px 20px rgba(47,90,42,0.4); transform: translateY(-1px); }
  &:active { transform: translateY(0); }
`;

/* ── Empty ── */

const EmptyWrap  = styled.div`flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 48px 24px; text-align: center;`;
const EmptyIcon  = styled.div`font-size: 4rem; margin-bottom: 16px;`;
const EmptyTitle = styled.p`margin: 0 0 8px; font-size: 1.1rem; font-weight: 800; color: #1a3318;`;
const EmptyDesc  = styled.p`margin: 0 0 24px; color: #9ca3af; font-size: 0.9rem;`;
